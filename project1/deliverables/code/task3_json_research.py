...
with open("../task3_stat.json", "r") as read_file:
...
data_set_partial_sorted = sorted(data_set_partial, key=lambda x: x['desc'], reverse=True)
...
print("data_set_gd_alg_old len = ", len(data_set_gd_alg_old))
...
data_set_gd_alg_old_sorted = sorted(data_set_gd_alg_old, key=lambda x: (x['mse_for_weight_vector'], x['epsilon_power'], x['eta_power']), reverse=True)
...
for data_point in data_set_gd_alg_sorted:
    print(data_point['mse_for_weight_vector'], data_point['epsilon_power'], data_point['distance_rate_power'], data_point['eta_power'], "\n", data_point['W'], data_point['T_Robbins_Monroe_pow'], "\n\n")
    #pass
