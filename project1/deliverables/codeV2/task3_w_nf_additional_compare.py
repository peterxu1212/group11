...
with open('../task3_w_wf_additional_compare.txt','w', buffering=1) as fout_t2r:
    ...
    for b_without_punctuation in (False,):

        for b_adv_feature_replace_original_feature in (False, True):
            
            for i_adv_feature_setting in (0, 2, 3, 4):
    ...
    with open('../task3_w_nf_additional_compare_stat.txt','w', buffering=1) as fout_t3rs:
        fout_t3rs.write(str_for_statistics)
        
    with open("../task3_w_nf_additional_compare_stat.json", "w") as wf_json_set:
